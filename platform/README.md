...
这里需要注意的是 jsBundle 的 moduleName，默认的 moduleName 为 "taroMobile"，需要和 `rn_temp/app.json` 里面的 name 字段保持一致。该配置在 `AppDelegate.m` 文件中。
...
同样，Android 这边默认的 jsBundle moduleName 也是 “taroMobileTemplate”，位于 `MainActivity.java` 的文件里面：

```java
package com.taromobile;
